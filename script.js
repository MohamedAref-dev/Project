...
// وظيفة لحذف الجهاز من البيانات والجدول
// دالة لتخريد الجهاز وحذف بياناته
function scrapDevice(deviceId) {
    const deviceIndex = devicesData.findIndex(device => device.id === deviceId);
    if (deviceIndex === -1) {
    ...
    if (!receiptInput || !receiptInput.files || receiptInput.files.length === 0) {
        alert('يرجى إرفاق إيصال الجرد قبل التخريد.');
        return;
    }
    ...
    alert('تم تخريد الجهاز بنجاح.');
    receiptInput.value = '';

    scrapNotifications.push({ deviceId: deviceId, message: 'تم تخريد الجهاز رقم ' + deviceId, read: false });
...
function updateNotificationBadge() {
    ...
    const unreadCount = scrapNotifications.filter(n => !n.read).length;
    if (unreadCount > 0) {
